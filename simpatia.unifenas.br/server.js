...
e interagindo com um chat baseado em Inteligência Artificial (Google Gemini).
...
R: As referências são geradas pela IA (Gemini). Recomenda-se que o professor revise
...
R: O módulo Plano de Aula usa o Google Gemini (modelo gemini-2.5-flash) para gerar
...
//Endpoint para o tutorial de perguntas e respostas
app.post("/api/tutorial", async (req, res) => {
  ...
  try {
    const completion = await groq.chat.completions.create({
      model: "llama-3.1-8b-instant", // Rápido e gratuito no tier free do Groq
      messages: [
        {
          role: "system",
          content: SIMPATIA_KNOWLEDGE,
        },
        {
          role: "user",
      ...
      temperature: 0.4,      // Respostas mais precisas e consistentes
      max_tokens: 512,        // Respostas concisas para o pop-up
      top_p: 0.9,
    });
    ...
    // Mensagem amigável se a chave ainda não foi configurada
